    ...
    <section id="tournament">
        ...
        <div class="form-row mb-5">          
            <div id="teams" class="col-md-12 sel">              
                ...
                @role('streamer')
                    @for ($i = 0; $i < count($tournament->teams); $i++)  
                        {{$i+1}} => {{$tournament->teams[$i]['code']}}<br>
                    @endfor 
                @endrole                 
                
                @if($team && isset($tournament->teams[$team-1]) && isset($tournament->teams[$team-1]['code']))
                    <div> Ваш код для входа в турнир: {{ $tournament->teams[$team-1]['code'] }}</div>
                @endif
                <div class="form-row mt-5">
                    @for ($i = 0; $i < count($tournament->teams); $i=$i+2)    
                        @php
                            $team_1 = isset($tournament->teams[$i])   ? $tournament->teams[$i]  ['team'] : null;
                            $team_2 = isset($tournament->teams[$i+1]) ? $tournament->teams[$i+1]['team'] : null;

                            $win_1 = '';
                            $win_2 = '';

                            if(isset($tournament->teams[$i]['win']))  
                                $win_1 = 'style=color:#de69c4';   
                            if(isset($tournament->teams[$i+1]['win']))
                                $win_2 = 'style=color:#de69c4'; 
                                                                     
                        @endphp    
                        <div class="col-md-4 lobby">
                            <h5 class="text-center">Игра #{{ intdiv($i, 2) + 1 }}</h5>
                            <input type="hidden" value="{{ $tournament->teams[$i]['mmr'] }}">
                            <input type="hidden" value="{{ $team_2 ? $tournament->teams[$i+1]['mmr'] : '' }}">
                            <div class="form-row">                                                                
